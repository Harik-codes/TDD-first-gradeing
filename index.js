...
export function near100(n, distance) {
  if (Math.abs(100 - n) <= distance) return true;
  else return false;
}
...
export function shareLastDigit(num1, num2) {
  let digitOne = num1 % 10;
  let digitTwo = num2 % 10;
  if (digitOne === digitTwo) return true;
  else return false;
}
...
export function shouldAnswerPhone(isMorning, isBoss, isAsleep) {
  ...
  if (isMorning && isBoss) return true;
